...
namespace WindowsFormsApplication1
{
    public partial class Historial_Cliente : Form
    {
        ...
        {
            rtb.Text = TextoAyuda.Historial_Cliente;
        ...
        private void Buscar_Click(object sender, EventArgs e)
        {
            if (ini && fin)
            {
                ...
                using (SqlConnection con = new SqlConnection("Server=localhost; database=INVERSEC_3; integrated security=yes"))
                {
                    try
                    {
                        SqlCommand comando = new SqlCommand("SELECT p.nombre Nombre,p.rut_emp Rut,f.numero_fact Factura,f.fecha,sum(fb.cantidad) Cantidad,f.total Total FROM CLIENTE p join FACTURA f on p.rut_emp=f.rut_cli join FACT_PRODUCTO fb on fb.num_fact=f.num_fact and f.fecha  between @fechaini and @fechafin group by p.nombre,p.rut_emp,f.numero_fact,f.fecha,f.total ", con);
            ...
            {
                MessageBox.Show("Debe ingresar dos fechas para buscar las facturas");
            }
        }
        ...
        private void Historial_Cliente_FormClosing(object sender, FormClosingEventArgs e)
        {
            this.Owner.Show();
        }
        ...
        private void Historial_Cliente_Paint(object sender, PaintEventArgs e)
        {
