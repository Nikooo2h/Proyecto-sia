...
namespace WindowsFormsApplication1
{
    public partial class FacturasCliente : Form
    {
        ...
        private void FacturasCliente_Load(object sender, EventArgs e)
        {
            using (SqlConnection con = new SqlConnection("Server=localhost; database=INVERSEC_3; integrated security=yes"))
            {
                ...
                try
                {
                    ...
                    comando = new SqlCommand("select p.titular,p.rut_emp,p.nombre,g.nombre giro,p.direccion,p.fono,p.fax,p.e_mail FROM Cliente p,giro g WHERE p.rut_emp=@rut AND p.codigo_giro=g.codigo", con);
                    ...
                    comando.CommandText = "select CONVERT(VARCHAR,fecha,105) fecha,iva,neto,total  from FACTURA where numero_fact=@n and rut_cli=@rut";
                    ...
                    comando.CommandText = "select fp.cod_pro,p.nombre,fp.descripcion,fp.cantidad,fp.precio from   FACTURA f join FACT_PRODUCTO fp on f.rut_cli=@rut and numero_fact=@n  and guia='factura' and fp.num_fact=f.num_fact join PRODUCTO p on fp.cod_pro=p.cod_pro";
                    da.SelectCommand = comando;
                    da.Fill(dtfp);
        ...
        private void FacturasCliente_FormClosing(object sender, FormClosingEventArgs e)
        {
            this.Owner.Show();
        }

        private void FacturasCliente_Paint(object sender, PaintEventArgs e)
        {
